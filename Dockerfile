...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    ldflags="-s -w \
      ...
      -X tailscale.com/version.gitCommitStamp=${commit}" && \
    CGO_ENABLED=0 GOOS="${TARGETOS}" GOARCH="${TARGETARCH}" \
      go build -trimpath -ldflags="${ldflags}" -o /out/tailscale ./cmd/tailscale && \
    CGO_ENABLED=0 GOOS="${TARGETOS}" GOARCH="${TARGETARCH}" \
      go build -trimpath -ldflags="${ldflags}" -o /out/tailscaled ./cmd/tailscaled && \
    CGO_ENABLED=0 GOOS="${TARGETOS}" GOARCH="${TARGETARCH}" \
      go build -trimpath -ldflags="${ldflags}" -o /out/containerboot ./cmd/containerboot
